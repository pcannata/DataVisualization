...
# dplyr::filter
diamonds %>% dplyr::select(cut, clarity) %>% dplyr::filter(cut == "Good") %>% tbl_df # Equivalent SQL: dplyr::select cut, clarity from diamonds where cut = 'Good';
diamonds %>% dplyr::select(cut, clarity) %>% dplyr::filter(cut %in% c("Good", "Fair")) %>% tbl_df # Equivalent SQL: dplyr::select cut, clarity from diamonds where cut in ('Good', 'Fair');# or Equivalent SQL:  dplyr::select cut, clarity from diamonds where cut = 'Good' or cut = 'Fair';
diamonds %>% dplyr::select(cut, clarity) %>% dplyr::filter(cut %in% c("Good", "Fair"), clarity == "VS1") %>% tbl_df # Equivalent SQL:  dplyr::select cut, clarity from diamonds where (cut = 'Good' or cut = 'Fair') and clarity = 'VS1';
diamonds %>% dplyr::select(cut, clarity) %>% dplyr::filter(cut %in% c("Good", "Fair"), clarity == "VS1" | is.na(cut)) %>% tbl_df # Equivalent SQL:  dplyr::select cut, clarity from diamonds where ((cut = 'Good' or cut = 'Fair') and clarity = 'VS1') or cut is null;
# diamonds %>% dplyr::select(cut, clarity) %>% dplyr::filter(carat > 2) %>% tbl_df # This will give an error
# Equivalent SQL:  dplyr::select cut, clarity  from diamonds  where carat > 2;
diamonds %>% dplyr::filter(carat > 2) %>% dplyr::select(cut, clarity) %>% tbl_df # This does not give an error.
diamonds %>% dplyr::select(carat, clarity) %>% dplyr::filter(carat > 2) %>% tbl_df # Equivalent SQL:  dplyr::select carat, clarity  from diamonds  where carat > 2;
...
diamonds %>% dplyr::arrange(carat) %>% tbl_df # Equivalent SQL:  dplyr::select * from diamonds order by carat;
diamonds %>% dplyr::arrange(desc(carat)) %>% tbl_df # Equivalent SQL:select * from diamonds order by carat desc;

# dplyr::rename
diamonds %>% dplyr::rename(tbl= table) %>% tbl_df # Equivalent: dplyr::select tbl as "table" from diamonds;

# dplyr::mutate
diamonds %>% dplyr::select(cut, clarity, x, y, z) %>% dplyr::filter(cut %in% c("Good", "Fair"), clarity == "VS1" | is.na(cut)) %>% dplyr::mutate(sum = x+y+z) %>% tbl_df # Equivalent: dplyr::select cut, clarity, x, y, z, x+y+z as sum from diamonds where ((cut = 'Good' or cut = 'Fair') and clarity = 'VS1') or cut is null
...
# head(diamonds) %>% dplyr::select(ID, cut, color) %>% tidyr::gather(variable, value, -ID) %>% tidyr::gather(variable, value) %>% spread(variable, value) %>% tbl_df
